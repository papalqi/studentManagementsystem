$(function () {
	...
	$.ajax({
                ...
                success:function(data){
                    if(data.status == 200){
                    	...
                    	$.each(data.info, function(i,val){   
                    		n=n+1;
                    		if(n<9){
                    		var content = "<tr><td><a>"+val.scourseid+"</a></td><td>"+val.scoursename+"</td><td class='am-hide-sm-only'>"+val.scollegename+"</td><td>"+val.smajorname+"</td><td>"+val.sname+"</td><td class='couid_td'>"+val.iteachclassid+"</td><td><div class='am-btn-toolbar'><div class='m-btn-group am-btn-group-xs'><button class='am-btn am-btn-default am-btn-xs am-text-secondary change_butt'><span class='am-icon-pencil-square-o'></span> 查看留言</button></div></div></td></tr>";
           ...
           $(".change_butt").click(function(){
            ...
            document.course_leavemess_form.action="admin_leavemess_view.html";
            $(".am-form").submit();
        });
