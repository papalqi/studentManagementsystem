                ...
                success:function(data){
                    if(data.status == 200){
                    	...
                    	$.each(data.info, function(i,val){   
                    		n=n+1;
                    		if(n<9){
                    		var content = "<tr><td><input type='checkbox'></td><td><a class='stuid_td'>"+val.stuid+"</a></td><td>"+val.stuname+"</td><td class='am-hide-sm-only'>"+val.stumajor+"</td><td class='am-hide-sm-only'>"+val.stuclass+"</td><td><div class='am-btn-toolbar'><div class='m-btn-group am-btn-group-xs'><button class='am-btn am-btn-default am-btn-xs  view_butt'><span class='am-icon-copy'></span> 查看</button></div></div></td></tr>";
...
//                  	console.log(data.stuname);
                    	...
                    	document.student_manange_form.action="student_view_student.html";
                    	$(".am-form").submit();
